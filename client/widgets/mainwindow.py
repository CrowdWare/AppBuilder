...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        if self.default_path:
            if self.loadProject(self.default_path + "/App.qml"):
                #gen = Generator()
    ...
    def loadProject(self, filename):
        self.default_path = filename[0:-9] # - /App.qml
    ...
    def previewSite(self, content = None):
        ...
        if content:
            ...
            self.statusBar().showMessage("App has no pages or posts to preview.")
    ...
    def webViewLoadFinished(self, success):
        if success:
            ...
            QMessageBox.warning(self, "AppBuilder", "Unable to open page.")
    ...
    def buildSite(self):
        ...
        if len(self.site.pages) == 0 and len(self.site.posts) == 0:
            self.statusBar().showMessage("App has no pages or posts to build.")
        else:
            gen = Generator()
            gen.generateSite(self, self.site)
